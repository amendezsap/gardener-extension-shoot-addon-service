...
- **Seed renders** (runtime cluster): Jobs are applied directly by the actuator with two-tier deduplication. Tier 1: state-based — spec hashes are tracked in the `seed-addon-state` ConfigMap. If the hash matches, skip entirely without checking the cluster (handles TTL-deleted Jobs). Tier 2: cluster-based — if no state, check the cluster for an existing Job with a matching `shoot-addon-service/spec-hash` annotation. Hook Secrets are applied directly first (create-or-skip) to ensure they exist before Jobs run. The chart's `ttlSecondsAfterFinished` is honored — Kubernetes can clean up completed Jobs without triggering recreation. Newly created Jobs are polled for completion (120s timeout, first deploy only).
